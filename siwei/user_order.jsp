				...
				<div class="bs-docs-example">
					...
					<table class="table table-bordered table-striped">
						...
						<c:forEach var="list" items="${list}">
							<tr>
								<td><fmt:formatDate value="${list.paydate}" pattern="yyyy/MM/dd  HH:mm:ss"/><br>${list.orderno}</td>
								<td>
									摇钱树1号&nbsp;${list.num1}<br>
									摇钱树8号&nbsp;${list.num2}<br>
									摇钱树9号&nbsp;${list.num3}<br>
								...
								<td>
									<c:if test="${list.status == 1}">
										<a href="/sw_user_pay_order.shtml?id=${list.id}" class="btn btn-small btn-success">确认付款</a>
									</c:if>
									<c:if test="${list.status == 3}">
										<a href="/sw_user_comfirm_deliver.shtml?id=${list.id}" class="btn btn-small btn-success">确认收货</a>
									</c:if>
					...
					<blockquote>
						<small>注：订单状态分为：待付款，已付款（付款后已经过几天），已发货（发货后已经过几天），已收货 <br>操作对应分为：待验证，请确认付款（开始制作15日左右发货），确认发货，提醒用户收货（7天后自动确认收货），确认收货</small>
					</blockquote>
